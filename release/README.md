...
`build-<arch>` runs the `docker-buildx` plugin for one architecture and pushes `YYYY-MM-<version>-<os>-<arch>`.
`merge-<platform-set>` composes `YYYY-MM-<version>-<os>` from those tags with the plugin's manifest mode, grouped by platform set so an amd64-only environment never lists an arm64 source that was never built.
...
That is why `2026-07` builds fourteen environments and `2026-08` builds eighteen from the same files.

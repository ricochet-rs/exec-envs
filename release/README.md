...
Each build workflow runs the `docker-buildx` plugin for one architecture and pushes `YYYY-MM-<version>-<os>-<arch>`, and the merge workflow composes `YYYY-MM-<version>-<os>` from those tags with the plugin's manifest mode.
